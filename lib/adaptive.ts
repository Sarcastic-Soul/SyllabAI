...
export async function calculateCourseMastery(
  ...
  try {
    ...
    const reviewedCards = cardList.filter(
      (c) => (c.interval ?? 0) > 0 || (c.easeFactor ?? 250) !== 250
    ).length;
    ...
    for (const card of cardList) {
      ...
      const easeRatio = Math.min(1.5, Math.max(0.4, easeFactor / 250));
      const intervalBonus = Math.min(1.3, 1 + interval / 10);
      const cardScore = Math.min(100, Math.round(easeRatio * intervalBonus * 60));
      weightedSum += cardScore;

      if (easeFactor < 220 || interval === 0) {
        weakConcepts.push({
          front: card.front,
          back: card.back,
    ...
    console.error("Error calculating course mastery:", error);
    return {
      totalCards: 0,
      reviewedCards: 0,
      masteryScore: 0,
